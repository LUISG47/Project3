...
function createFeatures(meteoriteData) {
    // Filter and sort the meteorite data based on fall date
    let filteredMeteorites = meteoriteData.filter(meteorite => meteorite[12] === "Fell")
        .map(meteorite => ({
            ...
            longitude: parseFloat(meteorite[15]), // Longitude
            latitude: parseFloat(meteorite[14]) // Latitude
        }))
        .sort((a, b) => b.year - a.year) // Sort by year (most recent first)
        .slice(0, 300); // Limit to the last 300 meteorites

    console.log("Filtered Meteorites:", filteredMeteorites); // Log the filtered meteorites to check
